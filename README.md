...
**Вивід `04_search.py`:**
![04a](results_screenshots/04_search.JPG(1).JPG)
![04b](results_screenshots/04_search.JPG(2).JPG)
![04c](results_screenshots/04_search.JPG(3).JPG)
...
![05a](results_screenshots/05_chunking%20(1).JPG)
![05b](results_screenshots/05_chunking%20(2).JPG)
![05c](results_screenshots/05_chunking%20(3).JPG)
![05d](results_screenshots/05_chunking%20(4).JPG)
...
**Яка стратегія дає більш осмислені чанки?** Семантична. Кожен її чанк складається з цілих речень, тому є завершеним висловлюванням. У результатах пошуку це видно: семантичні чанки читаються як зв'язний текст, тоді як fixed-size чанки часто починаються або закінчуються посеред речення.
...
**Як розмір overlap впливає на кількість чанків і покриття тексту?** Крок вікна дорівнює chunk_size − overlap, тому кількість чанків ≈ довжина_тексту / (chunk_size − overlap). У нашому експерименті fixed-size дав 211 чанків, semantic — 205. Overlap 15 слів із 60 збільшує кількість чанків приблизно на третину порівняно з розбиттям без перекриття, але гарантує краще покриття.
...
![06a](results_screenshots/06_hybrid_search%20(1).JPG)
![06b](results_screenshots/06_hybrid_search%20(2).JPG)
![06c](results_screenshots/06_hybrid_search%20(3).JPG)
![06d](results_screenshots/06_hybrid_search%20(4).JPG)

## Порівняльна таблиця: 3 запити × 3 методи

| Запит | BM25 (топ-1) | Vector (топ-1) | Hybrid/RRF (топ-1) | Хто виграв і чому |
|---|---|---|---|---|
| "BERT fine-tuning" | The NMSSM Solution to the Fine-Tuning Problem (11.5) | Misere quotients for impartial games (rank 1) | The NMSSM Fine-Tuning Problem [B-] | BM25: токен "fine-tuning" знайшов статті з цим терміном |
...
| "making computers understand human emotions from text" | Chinese Text Entry (18.3) | Opinion Dynamics (rank 1) | Text Input Method [BV] | Hybrid: документ присутній в обох методах з найвищим RRF |

## Теоретичні відповіді

**Який метод дав кращий результат і чому?** Залежить від типу запиту. BM25 на запиті "BERT fine-tuning" знайшов статті з точним токеном "fine-tuning" — це класична перевага лексичного пошуку. На запиті "making computers understand human emotions from text" гібрид виграв: знайшов "On the Development of Text Input Method" з RRF=0.0323 [BV] — документ присутній і в BM25, і у векторному топ-5, що дало найвищий консенсусний скор.
